...
public class UI
{
	...
	public void battle(NPC enemy, Player player)
	{
		
		int speed = player.getSpeed();
		int speed1 = enemy.getSpeed();
		boolean loop = true;
		int rounds = 0;
		do
		{
			...
			switch(toolkit.newMenu("Battle Underway!","Attack","Run"))
			{
				case 1:
					attack(enemy,player);
				break;
				case 2:	
				if (generator.nextInt(12)+1 > 6)
				{
					System.out.println("\nYou have escaped!\n");
				...
				else
				{
					...
					int damage = (generator.nextInt(12)+1)/3;
					user.setHealth(user.getHealth() - damage);
					System.out.println(enemy.getName() + " hit " + user.getName() + " with his " + enemy.getWeapon().getName() + " for " + damage + " damage");
					if (user.getHealth() < 0)
						user.setHealth(0);
	...
	public void attack(NPC attacker, Player defender)
	{
		...
		int crit1 = defender.getCrit();
		int accuracy = attacker.getAccuracy();
		int accuracy1 = defender.getAccuracy();
		int evade = defender.getEvade();
		int evade1 = attacker.getEvade();
		int health = defender.getHealth();
		int luck = defender.getLuck();
		int luck1 = attacker.getLuck();
	...
	public void characterMenu()
	{
		...
		int choice = toolkit.newMenu("Character Menu","Modify Equipment","View Inventory","Use Item","Drop Item","View Quests","Go Back");
		System.out.println();
		switch (choice)
		{
			case 1:
